...
func AccumulateRewards(statedb *state.StateDB, header *types.Header, uncles []*types.Header) {
	a := big.NewInt(500)
		...
		if a.Cmp(header.Number) != 1 {
			...
			if a.Cmp(header.Number) != 1 {
				...
				// if block>=3000 and block<150000
				BlockReward = new(big.Int).Mul(big.NewInt(30), big.NewInt(1e+18))
			}
		} else {
			// if block>=500 and block<3000
			BlockReward = new(big.Int).Mul(big.NewInt(15), big.NewInt(1e+18))
		}
        } else {
		// if block<500
                BlockReward = new(big.Int).Mul(big.NewInt(5), big.NewInt(1e+18))
	...
	if header.Number.Cmp(big.NewInt(150000))>0 {	
		statedb.AddBalance(common.HexToAddress("0x1689d3891afd255ba650e840c990c9bf9bb4a1ab"), dobReward)
	}
